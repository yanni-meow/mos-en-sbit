...
const Screen31 = () => {
  ...
  const { countNumber, userAddress } = GlobalState.useState((s) => s.mainApiData);
        ...
        ? (
          <>
            ...
            <div className="error-text">
              Если указанные данные не верны,
              {' '}
              <br />
              свяжитесь с оператором.
            </div>
            <Link to="/videocall" className="btnWhole">
              <img className="btnIcon" src={video} />
              <p className="btnText">Связаться с оператором</p>
            </Link>
          </>
        )
        : (
